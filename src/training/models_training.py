...
class ModelTrainer:
    ...
    def train_bagging_classifier(self):
        estimator = BaggingClassifier(random_state=self.random_state, n_jobs=self.n_jobs)
        parameters = {
    ...
    def train_ada_boost_classifier(self):
        ...
        parameters = {
            'estimator__base_estimator': [DecisionTreeClassifier(max_depth=d, max_leaf_nodes=9,
                                                                min_impurity_decrease=0.001,
                                                                min_samples_leaf=l, random_state=self.random_state) 
                                          for d in [3, 5, 8] for l in [8, 10]],
    ...
    def _perform_randomized_search(self, estimator, parameters):
        grid_obj = RandomizedSearchCV(estimator, n_iter=self.n_iter_search, param_distributions=parameters,
                                      scoring=self.acc_scorer, cv=5, n_jobs=self.n_jobs)
