...
from src.feature_engineering.model_specific_transformations import log_transform_features,perform_upsampling, perform_downsampling, encode_categorical_features, apply_one_hot_encoding, replace_values, split_data
...
def test_log_transform_features():
    ...
    features = ['Credit_Limit', 'Avg_Open_To_Buy', 'Total_Amt_Chng_Q4_Q1', 'Total_Trans_Amt', 'Total_Ct_Chng_Q4_Q1', 'Avg_Utilization_Ratio']
    expected_output = {
    ...
    # Assert
    pd.testing.assert_frame_equal(result_df, pd.DataFrame(expected_output), check_dtype=False)
    
    
def test_encode_categorical_features():
    # Arrange
    test_data = {
        'Attrition_Flag': ['Existing Customer', 'Attrited Customer', 'Existing Customer', 'Attrited Customer'],
        'Gender': ['M', 'F', 'F', 'M']
    }
...
def test_split_data():
    ...
    # Act
    X_train, X_test, y_train, y_test = split_data(test_df.copy(), target_column, test_size, random_state)
...
@patch('src.feature_engineering.model_specific_transformations.SMOTE', return_value=mock_smote_instance)
def test_perform_upsampling(mock_smote_class):
    # Call the function
    X_res, y_res = perform_upsampling(X_train, y_train)

    # Assert that SMOTE was instantiated and fit_resample was called with the right arguments
    mock_smote_class.assert_called_once_with(sampling_strategy=1, k_neighbors=5, random_state=1)
    mock_smote_instance.fit_resample.assert_called_once_with(X_train, y_train.ravel())
...
def test_perform_upsampling(mock_smote_class):
    ...
    np.testing.assert_array_equal(mock_smote_instance.fit_resample.call_args[0][0], X_train.to_numpy())
    np.testing.assert_array_equal(mock_smote_instance.fit_resample.call_args[0][1], y_train.to_numpy())