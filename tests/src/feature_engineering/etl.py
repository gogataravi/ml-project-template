...
def test_load_data():
    ...
    with patch("src.feature_engineering.etl.validate_missing_data", autospec=True) as mock_validate:
...
def test_validate_missing_data():
    ...
    with pytest.raises(ValueError, match=r"Columns \['A'\] have more than 20.0% missing values."):
